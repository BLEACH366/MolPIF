...
def keep_scaffold_and_groups(mol, scaffold, attachment_atoms):
    ...
    # Draw.MolToFile(emol, os.path.join('./output_partA/frag_part_filter', 'test2.png'))
    ...
    # Draw.MolToFile(emol, os.path.join('./output_partA/frag_part_filter', 'test3.png'))
...
def process_sdf_files(molecule_sdf, scaffold_sdf, attachment_atoms):
    """
    处理SDF文件,保留骨架和指定基团
    ...
    """
    # 读取骨架和小分子
    scaffold_supplier = Chem.SDMolSupplier(scaffold_sdf)
    molecule_supplier = Chem.SDMolSupplier(molecule_sdf)

    
    # 假设骨架文件中只有一个分子
    scaffold = scaffold_supplier[0]
...
if __name__ == '__main__':
    data_dir = "."
    gen_dir = 'output_partA3'
    scaffold_sdf = './m-BMF219_ligand_A.sdf'
    attachment_atoms = [15]
    min_add_num = 7
