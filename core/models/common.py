...
def compose_context(h_protein, h_ligand, pos_protein, pos_ligand, batch_protein, batch_ligand, gen_flag_lig=None):
    ...
    if gen_flag_lig is None:
        ...
        mask_gen = torch.cat([
            torch.zeros([batch_protein.size(0)], device=batch_protein.device).bool(),
            (torch.ones([batch_ligand.size(0)], device=batch_ligand.device) * gen_flag_lig).bool(),
        ], dim=0)[sort_idx]
        return h_ctx, pos_ctx, batch_ctx, mask_ligand, mask_gen
