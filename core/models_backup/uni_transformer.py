...
class UniTransformerO2TwoUpdateGeneral(nn.Module):
    ...
    def _connect_edge(self, x, mask_ligand, batch, b_idx):
        if self.cutoff_mode == 'radius':
            ...
            # edge_index = knn_graph(x, k=max(4, self.knn - b_idx * 4), batch=batch, flow='source_to_target')
    ...
    def forward(self, h, x, mask_ligand, batch, return_all=False, fix_x=False):
        ...
        for b_idx in range(self.num_blocks):
            edge_index = self._connect_edge(x, mask_ligand, batch, b_idx)
            src, dst = edge_index
        ...
        # # 假设 all_x[-2] 的形状为 (N, 3)
        # # 分别取出 ligand 和非 ligand 对应的特征
        # trans_all = all_x[-1][mask_ligand] * 0.1    # 形状：(N_l, 3)，N_l为ligand的点数
        # rotvec_all = all_x[-1][~mask_ligand] * 0.1   # 形状：(N_r, 3)，N_r为非ligand的点数

        # # 同时根据 mask 对应的 batch 索引（假设 batch 的形状为 (N,)）
        # batch_trans = batch[mask_ligand]  # 对应 ligand 的 batch 索引
        # batch_rot = batch[~mask_ligand]   # 对应非ligand 的 batch 索引

        # # 采用 scatter_mean 将每个 batch 内的向量取平均，得到 (B, 3) 的张量
        # trans = scatter_mean(trans_all, batch_trans, dim=0)  # 平移向量 (B, 3)
        # rotvec = scatter_mean(rotvec_all, batch_rot, dim=0)    # 旋转向量 (B, 3)

        # # 定义 Rodrigues 公式，将旋转向量转换为旋转矩阵
        # def rodrigues(rotvec):
        #     # rotvec 形状：(B, 3)
        #     theta = torch.norm(rotvec, dim=-1, keepdim=True)  # (B, 1)
        #     # 为避免 theta 过小导致数值不稳定，做一个小阈值处理
        #     theta = torch.where(theta < 1e-6, torch.ones_like(theta), theta)
        #     k = rotvec / theta  # 单位旋转轴 (B, 3)
            
        #     # 构造每个 batch 的 3x3 反对称矩阵
        #     K = torch.stack([
        ...
        #     I = torch.eye(3, device=rotvec.device).unsqueeze(0)  # (1, 3, 3)
        #     sin_theta = torch.sin(theta).unsqueeze(-1)  # (B, 1, 1)
        #     cos_theta = torch.cos(theta).unsqueeze(-1)  # (B, 1, 1)
        #     rotmat = I + sin_theta * K + (1 - cos_theta) * torch.bmm(K, K)  # (B, 3, 3)
        #     return rotmat

        # # 计算每个 batch 的旋转矩阵，rotmat 的形状为 (B, 3, 3)
        # rotmat = rodrigues(rotvec)

        # # 对于每个点 x（形状 (N, 3)），根据其所属的 batch 对应的旋转矩阵和平移向量进行转换
        # # 这里我们利用 batch 张量对 rotmat 和 trans 进行索引：
        # rotmat_per_point = rotmat[batch]   # (N, 3, 3)
        # trans_per_point = trans[batch]     # (N, 3)

        # # 更新 x：每个点先乘对应的旋转矩阵，再加上对应的平移向量
        # x_modified = torch.einsum('nij,nj->ni', rotmat_per_point, all_x[-2]) + trans_per_point

        # # 最终输出
        # outputs = {'x': x_modified, 'h': h}
