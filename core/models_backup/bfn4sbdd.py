...
class BFN4SBDDScoreModel(BFNBase):
    ...
    def interdependency_modeling(
        ...
        if gen_flag_lig is None:
            ...
            # for mask_ligand, ligand atom is True
            # ---------------------
            # time = 2 * time - 1
    ...
    def loss_one_step(
        ...
        mu_coord, gamma_coord = self.continuous_var_interpolation_update(
        ...
        if use_random_mask:
            if torch.rand(1) >= 0.5:
                ...
                mask = torch.rand_like(gen_flag_lig) < 0.5
    ...
    def sample(
        ...
        for i in trange(1, sample_steps + 1, desc=f'{desc}'):
            ...
            if self.sampling_strategy == "vanilla":
            ...
            elif "end_back" in self.sampling_strategy:  # self.sampling_strategy is end_back_pmf
                ...
                mu_pos_t = self.continuous_var_interpolation_update(t, x=coord_pred, sigma1=self.sigma1_coord)[0]
