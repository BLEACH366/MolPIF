...
class BFNBase(nn.Module):
    ...
    def continuous_var_interpolation_update(self, t, x, sigma1, prior=None):
        ...
        # noise = [torch.zeros_like(x).to(x.device), torch.ones_like(x).to(x.device)]

        gamma = 1 - torch.pow(sigma1, t)  # [0,1]
        # gamma = t  # [0,1]
        
        # x_noised = [x + (1 - gamma) * noise[0], 0.0 + (1 - gamma) * noise[1]]  # test2
        # x_noised = [x + torch.sqrt((1 - gamma)/gamma) * noise[0], 0.0 + torch.sqrt((1 - gamma)/gamma) * noise[1]]  # test

        # x_flow = [gamma * x_noised[0] + (1 - gamma) * prior[0], gamma * x_noised[1] + (1 - gamma) * prior[1]]
        x_flow = [gamma * x, (1 - gamma) * prior[1]]  # flow

        # mu = x_flow[0] + x_flow[1] * torch.randn_like(x).to(x.device)  # std = 1 - gamma**2
        mu = x_flow[0] + torch.sqrt(x_flow[1]) * torch.randn_like(x).to(x.device)  # std = sqrt(1 - gamma**2), test2
    ...
    def discrete_var_interpolation_update(self, t, x, K, sigma1, prior=None):
        ...
        # """
        if prior is None:
            # prior = torch.ones_like(x).to(x.device)  # test3
            prior = torch.ones_like(x).to(x.device) / K  # test2
        # noise = (1 / K - x) / 2

        gamma = 1 - torch.pow(sigma1, t)   # (B,)
        # gamma = t  # [0,1]

        # x_noised = x + (1 - gamma) * noise

        # x_flow = gamma / (1 - gamma) * x_noised +  prior  # test, test3
        # x_flow = gamma * x_noised + (1 - gamma) * prior  # test2
        x_flow = gamma * x + (1 - gamma) * prior  # flow
    ...
    def dtime4continuous_interpolation_loss(self, i, N, sigma1, x_pred, x, segment_ids=None):
        ...
        # weight = gamma ** 2 / (2 * (1 - gamma**2)**2)
        # weight = gamma ** 2 / (2 * (1 - gamma + torch.sqrt(gamma*(1-gamma)))**2)  # test
        # weight = gamma ** 2 / (2 * (1 - gamma**2))  # test2
        # weight = np.sqrt(N) * gamma ** 2 / (2 * (1 - gamma**2))  # test2
        # weight = 10 * gamma ** 2 / (2 * (1 - gamma**2))  # test3
        weight = np.sqrt(N) * gamma ** 2 / (2 * (1 - gamma))  # flow
    ...
    def dtime4discrete_interpolation_loss_prob(
        ...
        # alpha1 = gamma * (p_0 + (1 - gamma)*(1 / K - p_0) / 2) + (1 - gamma) * 1
        # alpha2 = gamma * (one_hot_x + (1 - gamma)*(1 / K - one_hot_x) / 2) + (1 - gamma) * 1

        # alpha1 = gamma/(1-gamma) * (p_0 + (1 - gamma)*(1 / K - p_0) / 2) + 1  # test3
        # alpha2 = gamma/(1-gamma) * (one_hot_x + (1 - gamma)*(1 / K - one_hot_x) / 2) + 1

        # alpha1 = gamma * (p_0 + (1 - gamma)*(1 / K - p_0) / 2) + (1 - gamma) * 1/K  # test2
        # alpha2 = gamma * (one_hot_x + (1 - gamma)*(1 / K - one_hot_x) / 2) + (1 - gamma) * 1/K

        alpha1 = gamma * p_0  + (1 - gamma) * 1/K  # flow
        ...
        if segment_ids is not None:
            ...
            # loss = scatter_mean(
            #     10 * dirichlet_kl_batch(alpha1, alpha2), segment_ids, dim=0  # test3_dirmore
            # )
        return loss
