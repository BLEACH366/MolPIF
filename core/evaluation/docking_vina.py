...
class VinaDockingTask(BaseDockingTask):
    ...
    @classmethod
    def from_generated_mol(cls, ligand_rdmol, ligand_filename, protein_root='./data/crossdocked', protein_path=None, **kwargs):
        if not protein_path:
            # load original pdb
            protein_fn = os.path.join(
    ...
    def run(self, mode='dock', exhaustiveness=8, **kwargs):
        ...
        if self.size_x * self.size_y * self.size_z > 27000:
            print(f'WARNING: box size {self.size_x * self.size_y * self.size_z} is too large, may cause memory error, skip docking')
